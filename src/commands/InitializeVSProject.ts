...
export const fileext="sfproj";
export async function openVSproject() {
  var x = await getWorkingFolder();
  var re = new RegExp("\."+ fileext +"$", "g");
  var files = find.fileSync(re, x);
  if (files.length) {
  var data = fs.readFileSync(files[0]);
  var isVS = 1;
  }
  else {
    window.showErrorMessage("This project doesn't contain an " + fileext+ ' ' + "file.");
    return;
  }
  ...
  for (i = 0; i < numofservices; i++) {
    ...
    var k;
    if (l < csprojinfo["Project"]["ItemGroup"].length) {
  ...
  await fs.writeFileSync(path.join(x, configfilename), JSON.stringify(values), (err) => {
    if (err) throw err;
  }
  );
